...
function ensureSodium() {
  if (!sodiumReadyPromise) {
    sodiumReadyPromise = import("libsodium-wrappers-sumo").then((mod) => {
...
export function getKey() {
  return ensureSodium().then((sodium) => {
    const slug = document
...
export function getKeyBase64() {
  return ensureSodium().then((sodium) => {
    return getKey().then((k) => sodium.to_base64(k))
  })
}
